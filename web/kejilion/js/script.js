...
function updateCPU() {
	...
	document.querySelector('.info-item .value:contains("系统负载")').textContent =
		loads.join(", ");
}
...
function updateMemory() {
	...
	document.querySelector(
		".memory-item:nth-child(1) .memory-value"
	).textContent = `${usedMem.toFixed(2)} MB`;
	document.querySelector(
		".memory-item:nth-child(2) .memory-value"
	).textContent = `${cacheMem.toFixed(2)} MB`;
	document.querySelector(
		".memory-item:nth-child(3) .memory-value"
	).textContent = `${availableMem.toFixed(2)} MB`;
	...
	document.querySelector(".progress-text").textContent = `${(
		usedMem + cacheMem
	).toFixed(2)}/${totalMem} MB (${totalUsedPercentage.toFixed(2)}%)`;
...
function updateSwap() {
	...
	swapText.textContent = `${formatSize(usedSwap)}/${formatSize(
		totalSwap
	)} (${percentage}%)`;
}
...
function updateDiskUsage() {
	...
	mountPoints.forEach((mount) => {
		...
		text.textContent = `${formatSize(used)}/${formatSize(
			total
		)} (${percentage}%)`;
	});
}
...
function updateNetwork() {
	...
	document.querySelector('.info-item .value:contains("总接收")').textContent =
		received;
	document.querySelector('.info-item .value:contains("总发送")').textContent =
		sent;
	...
	document.querySelector(
		'.info-item .value:contains("系统时间")'
	).textContent = `Asia/Shanghai ${timeString}`;
	...
	if (dnsStatus) {
		...
		dnsStatus.style.color =
			dnsLatency > 50 ? "var(--status-high)" : "var(--text-color)";
	}
}
...
async function editHostname() {
	const currentHostname = document.querySelector(
		'.info-item .value:contains("主机名")'
	).textContent;
...
function completeProgress(success, message) {
	...
	statusMessage.textContent = message;
	statusMessage.style.color = success
		? "var(--status-low)"
		: "var(--status-high)";
...
function submitSwapSize() {
	...
	if (
		document
			.querySelector(".swap-option-btn.custom")
...
function renderWebsiteList() {
	...
	websiteList.innerHTML = mockWebsites
		.map(
			(site) => `
				<tr>
						<td>${site.domain}</td>
...
function renderDatabaseList() {
	const databaseList = document.querySelector(
		"#database-tab .website-table tbody"
	);
	if (!databaseList) return;

	databaseList.innerHTML = mockDatabases
		.map(
			(db) => `
				<tr>
						<td>${db.name}</td>
...
function renderContainerList() {
	...
	containerList.innerHTML = mockContainers
		.map(
			(container) => `
				<tr>
						...
						<td>
								<span class="status-badge ${container.status === "running"
					? "status-running"
					: "status-stopped"
				}">
										${container.status}
...
function renderImageList() {
	...
	imageList.innerHTML = mockImages
		.map(
			(image) => `
				<tr>
...
function renderNetworkList() {
	...
	networkList.innerHTML = mockNetworks
		.map(
			(network) => `
				<tr>
						<td>${network.id}</td>
...
function renderVolumeList() {
	...
	volumeList.innerHTML = mockVolumes
		.map(
			(volume) => `
				<tr>
						<td>${volume.name}</td>
