...
func InitStore() error {
	...
	if err != nil {
		return fmt.Errorf("failed to open token store database: %w", err)
	}
	log.Printf("Token store database opened at: %s", dbPath)

	// Ensure buckets exist
	err = db.Update(func(tx *bolt.Tx) error {
		// Create tokens bucket if it doesn't exist
		_, err := tx.CreateBucketIfNotExists([]byte(tokensBucketName))
		if err != nil {
			return fmt.Errorf("failed to create tokens bucket: %w", err)
		}

		// Create revoked_tokens bucket if it doesn't exist
		_, err = tx.CreateBucketIfNotExists([]byte(revokedBucketName))
		if err != nil {
			return fmt.Errorf("failed to create revoked_tokens bucket: %w", err)
		}
	...
	err := db.View(func(tx *bolt.Tx) error {
		...
		return tokensBucket.ForEach(func(k, v []byte) error {
			...
			if err := json.Unmarshal(v, &info); err != nil {
				log.Printf("%s Error unmarshaling token info for JTI %s: %v. Skipping.", time.Now().UTC().Format(time.RFC3339), tokenID, err)
				return nil
			}
